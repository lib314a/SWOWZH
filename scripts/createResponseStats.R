# Description:
# Calculate response statistics for SWOW-ZH
...
# The same function achieved in MATLAB pipeline is "frequencyCalculating.m"
#
...
data.file           = './data/SWOW-ZH_R55.csv'
output.file         = './output/responseStats.SWOW-ZH.csv'
report.file         = './output/reports/responseStats.SWOW-ZH.rds'

# Replace #Missing and #Unknown with NA
SWOWZH              = read_csv(data.file)
SWOWZH$R1[which(SWOWZH$R1 == "#Unknown")] = NA
SWOWZH$R2[which(SWOWZH$R2 == "#Unknown")] = NA
SWOWZH$R3[which(SWOWZH$R3 == "#Unknown")] = NA
SWOWZH$R1[which(SWOWZH$R1 == "#Missing")] = NA
SWOWZH$R2[which(SWOWZH$R2 == "#Missing")] = NA
SWOWZH$R3[which(SWOWZH$R3 == "#Missing")] = NA
write.table(SWOWZH, data.file, row.names = FALSE, sep = ',', quote = FALSE)

results = list()

# Response frequencies for SWOW-ZH R123
response            = 'R123'
SWOWZH              = importDataSWOW(data.file,response)

# Calculate unknown and missing results
results$unknown   = SWOWZH %>% filter(RPOS == 'R1',is.na(response)) %>% nrow() / (SWOWZH %>% nrow()/3) * 100
results$missing.R2 = SWOWZH %>% filter(RPOS == 'R2',is.na(response)) %>% nrow() / (SWOWZH %>% nrow()/2) * 100
results$missing.R3 = SWOWZH %>% filter(RPOS == 'R3',is.na(response)) %>% nrow() / (SWOWZH %>% nrow()) * 200

SWOWZH              = SWOWZH %>% filter(complete.cases(response))
Freq.SWOW.R123      = SWOWZH %>%
                      group_by(response) %>%
                      summarise(Freq.R123 = n()) %>%
                      arrange(desc(Freq.R123))

Types.SWOW.R123     = SWOWZH %>%
